...
将 `config.template.yaml` 的内容复制到 `config.yaml`，并根据需要修改配置文件。
...
* `app` 模块：负责初始化和运行应用，包含了项目的入口函数和初始化函数。
...
`db` 包提供了函数 `GetORM` `GetSQL`，用于获取存储数据库连接对象。导出的 `Init` 函数来初始化数据库连接和迁移数据表。
...
`router` 包提供了一个 `Init` 函数，接受一个 `gin.Engine` 对象指针，用于初始化路由。
...
`main.go` 文件和 `app` 模块配置了项目的入口函数和初始化函数。

## 测试结果

在目录`test/videogen`下，运行 `go run videogen` 生成测试视频。运行参数请看源码。

先启动服务 `go run douyin`，然后在项目根目录下，运行 `go test` 进行单元测试和集成测试。
